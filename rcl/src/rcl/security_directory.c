  ...
  const rcl_allocator_t * allocator)
{
  ...
  if (1 == strlen(namespace_)) {
    node_secure_root = rcutils_join_path(ros_secure_root_env, name, *allocator);
  } else {
    char * node_fqn = NULL;
    ...
    node_fqn = rcutils_format_string(*allocator, "%s%s%s", namespace_, "/", name);
